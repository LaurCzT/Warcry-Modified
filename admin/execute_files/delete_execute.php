...
else if ($action == 'changelogs')
{
    ...
    //bind on success
    $ERRORS->onSuccess('The category was successfully deleted.', '/index.php?page=changelogs');

    if (!$id)
    {
        $ERRORS->Add("The changelog id is missing.");
    }

    $ERRORS->Check('/index.php?page=changelogs');

    ####################################################################
    ## The actual script begins here

    $delete = $DB->prepare('DELETE FROM `changelogs` WHERE `id` = :id LIMIT 1;');
    $delete->bindParam(':id', $id, PDO::PARAM_INT);
    $delete->execute();

    if ($delete->rowCount() < 1)
    {
        $ERRORS->Add("The website failed to delete the changelog record.");
    }
    else
    {
        unset($delete);
...
else if ($action == 'forum')
{
    ...
    $ERRORS->NewInstance('forums_delcat');
    //bind on success
    $ERRORS->onSuccess('The category was successfully deleted.', '/index.php?page=forums');
    ...
    $ERRORS->Check('/index.php?page=forum');

    ####################################################################
    ## The actual script begins here

    $delete = $DB->prepare('DELETE FROM `wcf_forums` WHERE `id` = :id LIMIT 1;');
    $delete->bindParam(':id', $id, PDO::PARAM_INT);
    $delete->execute();

    if ($delete->rowCount() < 1)
    {
        $ERRORS->Add("The website failed to delete the forum record.");
    }
    else
    {
        unset($delete);
