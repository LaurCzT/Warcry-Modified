...
(function($)
{
	var methods =
	{
		...
		init : function(options)
		{
			...
			if (typeof $this.data('exampleStore') == 'undefined')
			{
				$this.data('exampleStore', {config: null});

				//merge the defaults with the passed config				
				$this.data('exampleStore').config = $.extend({}, methods.defaults, config);
			}
			else
			{
				//merge the old config with the passed one
				$this.data('exampleStore').config = $.extend({}, $this.data('exampleStore').config, config);
			}
		
			var config = $this.data('exampleStore').config;
			...
			{
				$this.exampleStore('loadPage', 'first');
				return false;
			});
			...
			{
				$this.exampleStore('loadPage', 'prev');
				return false;
			});
			...
			{
				$this.exampleStore('loadPage', 'next');
				return false;
			});
			...
			{
				$this.exampleStore('loadPage', 'last');
				return false;
			});
			
			$('#store-empty-cart-btn').on('click', function()
			{
				$this.exampleStore('shoppingCart').empty();
				return false;
			});
			...
			$('#store-search-form').on('submit', function()
			{
				...
				$this.exampleStore('applyFilter', { search: $search, quality: $quality });
			...
			$('#store-complete-form').on('submit', function()
			{
				...
				{
					//no items, fail
					$.fn.exampleAlertBox('open', '<p>You did not select any items.</p>');
					return false;
				}
				...
				{
					//no characters, fail
					$.fn.exampleAlertBox('open', '<p>Unable to proceed, you dont have any characters.</p>');
					return false;
				}
				else
				{
					...
					{
						$.fn.exampleAlertBox('open', '<p>Unable to proceed, please select a character.</p>');
						return false;
					}
				}
				
				//check if the server is online... Im gonna do that in verifyPayment		
				$this.exampleStore('verifyAmount');
		...
		applyFilter : function(filter)
		{
			...
			//start the loading
			$this.exampleStore('startLoading');
			
			//queue the Filter info ajax
			exampleQueue('STORE').add(function()
			{
				...
				$.ajaxSetup({
					error:function(x,e)
					{
						if(x.status==408)
						{
							$this.exampleStore('applyFilter', $filter);
						}
						else
						{
				...
				$.get(
					...
					function(data)
					{
						...
						$('#store-item-container').exampleStore('changeConfig',
						{
							currentPage: 1, 
		...
		loadPage : function(page)
		{
			...
			$.ajaxSetup({
				error:function(x,e)
				{
					if(x.status==408)
					{
						$this.exampleStore('loadPage', $page);
					}
					else
					{
					...
					function(data)
					{
						...
						{				
							...
							list.find('item').each(function(i, e)
							{
								...
								exampleQueue('STORE').add(function()
								{
									$('#store-item-container').exampleStore('loadItem', { itemId: itemId, entry: entry, order: order, source: $config.source, pageItems: count });
								});
							});
						}
						...
						$('#store-item-container').exampleStore('updatePagination', { page: $page, count: count });
		...
		updatePagination : function(options)
		{
			...
			if ($('#store-pagination').css('display') != 'none')
			{
				...
				exampleQueue('STORE_NAVS').add(function()
				{
					$('#store-pagination').fadeOut('fast', function()
					{
						//continue the queue
						exampleQueue('STORE_NAVS').goNext();
					});
				});
			}
		...
		loadItem : function(options)
		{
			...
			$.ajaxSetup({
				error:function(x,e)
				{
					if(x.status==408)
					{
						$this.exampleStore('loadItem', $entry, $storeItem);
					}
			...
			$.get(
				...
				function(data)
				{
					...
					var inventorySlot = data.inventory;
					...
					{
						$this.exampleStore('shoppingCart').add({ itemId: $itemId, entry: $entry, name: $name, storeItem: $storeItem, icon: $icon, quality: $quality });
						return false;
					});
		...
		shoppingCart : function()
		{
			...
			var add = function(data)
			{
				...
				if ($silver.length > 0)
				{
					...
					{
						$this.exampleStore('shoppingCart').selectCurreny($cartItemId, 'silver');
						return false;
					});
				}
				if ($gold.length > 0)
				{
					...
					{
						$this.exampleStore('shoppingCart').selectCurreny($cartItemId, 'gold');
						return false;
					});
				}
				...
				{
					$this.exampleStore('shoppingCart').remove($cartItemId);
					return false;
				});
				...
				$this.exampleStore('shoppingCart').updateTotal();
			...
			//get the results
			  $.get(
				...
				function(data)
				{
					...
					{
						//prompt the error
						$.fn.exampleAlertBox('open', '<p>' + data + '</p>');
					}
				}
			);
		},	
  	...
  	$.fn.exampleStore = function(method)
  	{
  		if (methods[method])
		{
		...
		{
      		$.error( 'Method ' +  method + ' does not exist on jQuery.exampleStore');
    	}    
  	};
