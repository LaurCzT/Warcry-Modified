...
function HideCurrentMap()
{
	...
	exampleQueue('TELEPORTER').add(function()
	{
		//hide the map
		$CurrentMap.fadeOut('fast', function()
		{
			//continue the queue
			exampleQueue('TELEPORTER').goNext();
		});
	});
}
...
function OnTeleportSubmit(e)
{
	...
	if (!$SelectedPoint)
	{
		$.fn.exampleAlertBox('open', '<p>Unable to proceed, you did not select teleport location.</p>');
		return false;
	}
	...
	$.get(
		...
		function(data)
		{
			...
			else
			{
				...
				{
					$.fn.exampleAlertBox('open', '<p>The selected character does not meet the level requirement. The location requires a minimum of atleast ' + reqLevel + ' level.</p>');
				}
				else
				{
                    window.location = 'http://www.example.com/execute.php?take=teleport&character='+$SelectedCharacter+'&point='+$SelectedPoint;
				}
			}
		}
	);
