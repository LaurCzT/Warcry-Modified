	...
	<h2>GM Tickets</h2>
        ...
        <br /><br />
		  ...
		  <table class="datatable">
			...
			{
				$where = "WHERE `gm_ticket`.`closedBy` = '0'";
			}
				
			$res = $REALM_DB->prepare("SELECT `gm_ticket`.`id`,
										`gm_ticket`.`description`,
										`gm_ticket`.`playerGuid`,
										`gm_ticket`.`name`,
										`gm_ticket`.`createTime`,
										`gm_ticket`.`closedBy`,
										`gm_ticket`.`assignedTo`,
										`gm_ticket`.`comment`,
										`gm_ticket`.`viewed`
								FROM `gm_ticket`
								".$where."
								ORDER BY `gm_ticket`.`createTime` DESC;");
			$res->execute();
