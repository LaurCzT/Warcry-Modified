...
<nav id="secondary" class="disable-tabbing">
    <ul>
        <li class="current"><a href="index.php?page=changelogs">Changelogs</a></li>
    </ul>
</nav>
...
<div class="tab" id="maintab">
    <div class="column left twothird">
        ...
        <table id="datatable2" class="datatable datatable_editable">
            ...
            $res = $DB->query("SELECT * FROM `changelogs` ORDER BY id DESC;");
            ...
            if ($res->rowCount() > 0)
            {
                while ($arr = $res->fetch())
                {
                    echo '
                          <tr data-id="', $arr['id'], '">
                            <td>', $arr['revision'] ? $arr['revision'] : 'undefined', '</td>
                            <td>', $arr['changelog'] ? $arr['changelog'] : 'undefined', '</td>
                                                     <td>
                                <div style="width: 220px;">
                                    <input type="text" disabled="disabled" value="', $arr['text'], '" id="cat-editable-', $arr['id'], '" isOpen="false" style="float: left;">
                                    <div class="clear"></div>
                                </div>
                                <div id="cat-editable-', $arr['id'], '-infobox" class="datatable_editable-infobox">Error</div>
                            </td>
                            <td>', $arr['author'] ? $arr['author'] : 'undefined', '</td>
                            <td>',
                    $arr["time"]
                    ,'</td>
                            <td>
                              <span class="button-group">
                                <a href="javascript: void(0);" style="width:58px;" onclick="return editableDatatable(this, \'cat-editable-', $arr['id'], '\', ', $arr['id'], ');" class="button icon edit">Edit</a>
                                <a href="execute.php?take=delete&action=changelogs&id=', $arr['id'], '" onclick="return deletecheck(\'Are you sure you want to delete this changelog?\');" class="button icon remove danger">Remove</a>
                              </span>
                            </td>
                          </tr>';
                }
    ...
    <div class="column right third">
        ...
        <form action="execute.php?take=add_changelog" method="post" id="add_changelog">
            <section>
                <label>Revision*</label>

                <div>
                    <input type="text" placeholder="Required" class="required" name="revision" />
                </div>
            </section>
            ...
            <section>
                ...
                <div>
                    <input type="text" placeholder="Required" class="required" name="changelog" />
                </div>
            </section>
            ...
            <section>
                <label>Text*</label>
                <div>
                    <input type="text" placeholder="Required" class="required" name="text" />
                </div>
            </section>
            ...
            <section>
                <label>Author*</label>
                <div>
                    <input type="text" placeholder="Required" class="required" name="author" />
                </div>
            </section>
...
<script type="text/javascript">
    ...
    function save_CategoryData(id, record)
    {
        ...
        $.post(
            $configURL + "/admin/execute.php?take=edit_changelog",
            {
                id: record,
                text: input.val()
            },
