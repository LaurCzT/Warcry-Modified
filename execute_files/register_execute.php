...
######################################
######### Gender Check ##############
if($genderError = AccountsRegister::checkGender($gender)) {
...
$recruiter = 0;
	...
	//register
  	if ($accountId = server_Account::register($username, $password, $email, $expansion, $recruiter))
  	{
		...
		//insert web record
		$insert = $DB->prepare("REPLACE INTO `account_data` (`id`, `displayName`, `birthday`, `country`, `secretQuestion`, `secretAnswer`, `last_ip`, `reg_ip`, `last_login`, `last_login2`, `status`, `gender`) VALUES (:accid, :displayName, :birthday, :country, :secretQuestion, :secretAnswer, :lastip, :regip, '0000-00-00 00:00:00', :lastlogin2, :status, :gender);");
		...
		{				
			...
			$mail->Subject = "example Registration";
			$mail->Body    = $message;
		...
		$NOTIFICATIONS->SetText('Welcome and thank you for joining the example community.<br>Your example account has been automatically activated.<br>Please enjoy.');
