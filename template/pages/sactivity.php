				...
				<?php
					...
					if ($count > 0)
					{
						...
						while ($arr = $res->fetch())
						{
							...
							echo '
			                <li>
				                <p id="r-item" class="sac"><a class="', strtolower($CORE->getItemQualityString($item['Quality'])), '" href="', $config['WoWDB_URL'], '/?item=', $item['entry'], '" target="_newtab" rel="item=', $item['entry'], '">[', $item['name'], ']</a></p>
				                <p id="r-date" class="sac">', $arr['time'], '</p>
				                <p id="r-info" class="sac">', $arr['money'], '</p>
			                </li>';
