...
(function($)
{
	...
	var methods =
	{
		...
		init : function(options)
		{
			...
			if (typeof $this.data('exampleArmorsets') == 'undefined')
			{
				$this.data('exampleArmorsets', {config: null});

				//merge the defaults with the passed config				
				$this.data('exampleArmorsets').config = $.extend({}, methods.defaults, config);
			}
			else
			{
				//merge the old config with the passed one
				$this.data('exampleArmorsets').config = $.extend({}, $this.data('exampleArmorsets').config, config);
			}
		
			var config = $this.data('exampleArmorsets').config;
			...
			{
				$this.exampleArmorsets('loadPage', 'first');
				return false;
			});
			...
			{
				$this.exampleArmorsets('loadPage', 'prev');
				return false;
			});
			...
			{
				$this.exampleArmorsets('loadPage', 'next');
				return false;
			});
			...
			{
				$this.exampleArmorsets('loadPage', 'last');
				return false;
			});
			...
			$('#character-select').on('change', function()
			{
				...
				{
					//queue the fade out
					exampleQueue('ARMORSETS').add(function()
					{
					...
					exampleQueue('ARMORSETS').add(function()
					{
						$this.exampleArmorsets('applyFilter', { category: config.filter.category, character: $lastSelectedCharacter });
					});
					//run the queue
					exampleQueue('ARMORSETS').goNext();
				}
			...
			$('#armors-filter-select').on('change', function()
			{
				...
				if ($lastSelectedCharacter != '' && $lastSelectedCharacter != 0)
				{						
					$this.exampleArmorsets('applyFilter', { category: $filter, character: config.filter.character });
				}
			});
			...
			$('#armorset-purchase-form').submit(function()
			{
				...
				{
					//no characters, fail
					$.fn.exampleAlertBox('open', '<p>Unable to proceed, please select character.</p>');
					return false;
				}
				...
				{
					//no characters, fail
					$.fn.exampleAlertBox('open', '<p>Unable to proceed, please select armor set.</p>');
					return false;
				}
				
				$this.exampleArmorsets('verifyAmount');
		...
		applyFilter : function(filter)
		{
			...
			{			
				//start the loading
				$this.exampleArmorsets('startLoading');
			});
			...
			exampleQueue('ARMORSETS').add(function()
			{
				...
				$.ajaxSetup({
					error:function(x,e)
					{
						if(x.status==408)
						{
							$this.exampleArmorsets('applyFilter', filter);
						}
						else
						{
				...
				$.get(
					...
					function(data)
					{
						...
						$('#armorsets-container').exampleArmorsets('changeConfig',
						{
							currentPage: 1, 
		...
		loadPage : function(page)
		{
			...
			$.ajaxSetup({
				error:function(x,e)
				{
					if(x.status==408)
					{
						$this.exampleArmorsets('loadPage', $page);
					}
					else
					{
					...
					function(data)
					{
						...
						{				
							...
							list.find('armorset').each(function(i, e)
							{
								...
								{
									$this.exampleArmorsets('clickEvent', $(this));
								});
												
								//queue the load of the current item
								exampleQueue('ARMORSETS').add(function()
								{
									$('#armorsets-container').exampleArmorsets('loadItems', { armorset: armorsetId, order: order, source: $config.source, pageItems: count });
								});
							});
						}
						...
						$('#armorsets-container').exampleArmorsets('updatePagination', { page: $page, count: count });
		...
		loadItems : function(options)
		{
			...
			for (i = 0; i < items.length; i++)
			{
				...
				exampleQueue('ARMORSETS_'+$armorsetId).add(function()
				{
					...
					$.get(
						...
						function(data)
						{
							...
							//continue the queue
							exampleQueue('ARMORSETS_'+$armorsetId).goNext();
						}
					);
  	...
  	$.fn.exampleArmorsets = function(method)
  	{
  		if (methods[method])
		{
		...
		{
      		$.error( 'Method ' +  method + ' does not exist on jQuery.exampleArmorsets');
    	}    
  	};
