...
//create queue
var exampleQueues = [];
exampleQueues['internal'] = [];

var exampleQueue = function(queueName)
{
    var add = function(fnc)
	{
		if (typeof queueName != 'undefined')
		{
			if (typeof exampleQueues[queueName] == 'undefined')
			{
				exampleQueues[queueName] = []
			}
			exampleQueues[queueName].push(fnc);
		}
		else
		{
        	exampleQueues['internal'].push(fnc);
		}
    };
    ...
    var goNext = function()
	{
		if (typeof queueName != 'undefined')
		{
			...
			if ($(exampleQueues[queueName]).size() < 1)
			{
				return false;
			}
			
			var fnc = exampleQueues[queueName].shift();
			...
			{
				console.log('exampleQueue: There are no other functions in the queue "'+queueName+'".');
			}
		}
		else
		{
 			//check if we have some functions in the queue
			if ($(exampleQueues['internal']).size() < 1)
			{
				return false;
			}
			
       		var fnc = exampleQueues['internal'].shift();
			if (typeof fnc == 'function')
			{
        		fnc();
			}
			...
			{
				console.log('exampleQueue: There are no other functions in the queue.');
			}
		}
    };
	...
	var size = function()
	{
		if (typeof queueName != 'undefined')
		{
			return $(exampleQueues[queueName]).size();
		}
		else
		{
			return $(exampleQueues['internal']).size();
		}
	};
